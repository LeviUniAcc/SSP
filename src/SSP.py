...
class SSP:
    ...
    def __init__(self,
        ...
        ssp_space = HexagonalSSPSpace(domain_dim=2, ssp_dim=SSP_DIM, length_scale=5,
                                      domain_bounds=np.array([[0, RES_X], [0, RES_Y]]))

        # Generate xy coordinates
        x_coords, y_coords = torch.meshgrid(torch.arange(0, RES_X), torch.arange(0, RES_Y),
                                            indexing='ij')
        ...
        # self._print_image(RES_X, RES_Y)

        # combine all object ssps

        ## init empty environment
        global_env_ssp = np.zeros(SSP_DIM)
        ...
        # self._print_extracted_image(RES_X, RES_Y, SSP_DIM, global_env_ssp, ssp_grid, ssp_space, vocab)

        self.vector = "idk yet"

    def _print_extracted_image(self, RES_X_param, RES_Y_param, SSP_DIM_param, global_env_ssp_param, ssp_grid_param, ssp_space_param, vocab_param):
        for obj in self.grid_objects:
            obj_type = get_value_from_string(obj.name['type'])
            if obj_type == 'agent':
                obj_type = get_value_from_string(obj.name['type'])
                ...
                sims_map = sims.reshape((RES_X_param, RES_Y_param))
                ...
                plt.imshow(sims_map, extent=(0, RES_X_param, 0, RES_Y_param))
    ...
    def _print_image(self, res_x_param, res_y_param):
        # example visualisation--------------------------------------- remove
        fig, ax = plt.subplots()
        global_grid = np.zeros((res_x_param, res_y_param))
        plt.imshow(global_grid, extent=[0, 200, 0, 200], cmap='gray')  # Setze extent auf [0, 200, 0, 200]
        plt.xticks([0, 20, 40, 60, 80, 100, 120, 140, 160, 180, res_x_param])  # x-Achse bei 0, 100 und 200
        plt.yticks([0, 20, 40, 60, 80, 100, 120, 140, 160, 180, res_y_param])  # y-Achse bei 0, 100 und 200
        # Gitterlinien
        grid_color = (.3, .3, .3, .5)
        grid_spacing = 20  # Abstand der Gitterlinien in Pixeln
        for i in range(0, res_x_param, grid_spacing):  # Setze Gitterlinien im Abstand von 20
            plt.hlines(i + grid_spacing, 0, res_y_param, color=grid_color, linewidth=1.0)
            plt.vlines(i + grid_spacing, 0, res_x_param, color=grid_color, linewidth=1.0)
        # add elements
        for object_element in self.grid_objects:
            ...
            if get_value_from_string(object_element.name['type']) == "walls":
                ax.add_patch(
                    Rectangle((y_pos_nulled, res_x_param - x_pos_nulled), width=20, height=20, color='gray',
                              zorder=3))
            # agent
            if get_value_from_string(object_element.name['type']) == "agent":
                ax.add_patch(Polygon(
                    [[y_pos_nulled, res_x_param - x_pos_nulled], [y_pos_nulled, res_x_param - x_pos_nulled + 20],
                     [y_pos_nulled + 20, res_x_param - x_pos_nulled + 10]], closed=True, color='r', zorder=5))
            # objects
            if get_value_from_string(object_element.name['type']) == "objects":
                ax.add_patch(Circle((y_pos_nulled, res_x_param - x_pos_nulled), radius=10, color='g', zorder=5))
            if get_value_from_string(object_element.name['type']) == "home_entity":
                ax.add_patch(
                    Rectangle((y_pos_nulled, res_x_param - x_pos_nulled), width=15, height=15, color='m', zorder=5))
